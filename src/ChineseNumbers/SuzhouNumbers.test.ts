...
describe('SuzhouNumbers.generate', () => {
  ...
  describe('Large numbers', () => {
    ...
    test('should handle ten thousands', () => {
      expect(SuzhouNumbers.generate('12345', '', '', true)).toBe('〡二〣〤〥\n万');
    });

    test('should handle hundred thousands', () => {
      expect(SuzhouNumbers.generate('123456', '', '', true)).toBe('〡二〣〤〥〦\n十万');
    });

    test('should handle millions', () => {
      expect(SuzhouNumbers.generate('1234567', '', '', true)).toBe('〡二〣〤〥〦〧\n百万');
    });

    test('should handle very large numbers', () => {
      expect(SuzhouNumbers.generate('12345678', '', '', true)).toBe('〡二〣〤〥〦〧〨\n千万');
      expect(SuzhouNumbers.generate('123456789', '', '', true)).toBe('〡二〣〤〥〦〧〨〩\n億');
    });
  });
  ...
  describe('Complex scenarios', () => {
    test('should handle real-world numbers', () => {
      ...
      expect(SuzhouNumbers.generate('2024', '', '年', true)).toBe('〢〇〢〤\n千年');

      // Price 1299 yuan
      expect(SuzhouNumbers.generate('1299', '', '元', true)).toBe('〡二〩〩\n千元');
