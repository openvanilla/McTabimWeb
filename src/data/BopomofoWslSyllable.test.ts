import { BopomofoWslSyllable } from './BopomofoWslSyllable';

describe('BopomofoWslSyllable', () => {
  it('test fromKeys', () => {
    let syllable = BopomofoWslSyllable.fromKeys('1');
    expect(syllable.consonant).toBe('1');
    expect(syllable.middleVowel).toBe(undefined);
    expect(syllable.finalVowel).toBe(undefined);
    expect(syllable.tone).toBe(undefined);

    syllable = BopomofoWslSyllable.fromKeys('u');
    expect(syllable.consonant).toBe(undefined);
    expect(syllable.middleVowel).toEqual(['u']);
    expect(syllable.finalVowel).toBe(undefined);
    expect(syllable.tone).toBe(undefined);

    syllable = BopomofoWslSyllable.fromKeys('8');
    expect(syllable.consonant).toBe(undefined);
    expect(syllable.middleVowel).toBe(undefined);
    expect(syllable.finalVowel).toBe('8');
    expect(syllable.tone).toBe(undefined);

    syllable = BopomofoWslSyllable.fromKeys('3');
    expect(syllable.consonant).toBe(undefined);
    expect(syllable.middleVowel).toBe(undefined);
    expect(syllable.finalVowel).toBe(undefined);
    expect(syllable.tone).toBe('3');

    // WSL specific keys
    syllable = BopomofoWslSyllable.fromKeys('!');
    expect(syllable.consonant).toBe('!');
    expect(syllable.reading).toBe('ㆠ');

    syllable = BopomofoWslSyllable.fromKeys('U');
    expect(syllable.middleVowel).toEqual(['U']);
    expect(syllable.reading).toBe('ㆪ');

    syllable = BopomofoWslSyllable.fromKeys('*');
    expect(syllable.finalVowel).toBe('*');
    expect(syllable.reading).toBe('ㆩ');

    syllable = BopomofoWslSyllable.fromKeys('$');
    expect(syllable.tone).toBe('$');
    expect(syllable.reading).toBe('ㄅ, ㄉ, ㄍ, ㄏ');

    syllable = BopomofoWslSyllable.fromKeys('1u83');
    expect(syllable.consonant).toBe('1');
    expect(syllable.middleVowel).toEqual(['u']);
    expect(syllable.finalVowel).toBe('8');
    expect(syllable.tone).toBe('3');
  });

  it('test isValid', () => {
    let syllable = BopomofoWslSyllable.fromKeys('1');
    expect(syllable.isValid).toBe(false);

    syllable = BopomofoWslSyllable.fromKeys('u');
    expect(syllable.isValid).toBe(true);

    syllable = BopomofoWslSyllable.fromKeys('8');
    expect(syllable.isValid).toBe(true);

    syllable = BopomofoWslSyllable.fromKeys('3');
    expect(syllable.isValid).toBe(false);

    syllable = BopomofoWslSyllable.fromKeys('1u83');
    expect(syllable.isValid).toBe(true);

    // Standing alone consonants
    syllable = BopomofoWslSyllable.fromKeys('5');
    expect(syllable.isValid).toBe(true);

    syllable = BopomofoWslSyllable.fromKeys('Y');
    expect(syllable.isValid).toBe(true);
  });

  it('test keys', () => {
    let syllable = BopomofoWslSyllable.fromKeys('1u83');
    expect(syllable.keys).toBe('1u83');

    syllable = BopomofoWslSyllable.fromKeys('!U*$');
    expect(syllable.keys).toBe('!U*$');
  });

  it('test reading', () => {
    let syllable = BopomofoWslSyllable.fromKeys('1u83');
    expect(syllable.reading).toBe('ㄅㄧㄚ˪');

    syllable = BopomofoWslSyllable.fromKeys('!U*$');
    expect(syllable.reading).toBe('ㆠㆪㆩㄅ, ㄉ, ㄍ, ㄏ');
  });
});
